...
test('GET /rest/2fa/status', { signal: AbortSignal.timeout(timeout) }, async () => {
  await runner
    ...
    .run({
      ...
      headers: {
        Authorization:
          'Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjp7ImlkIjoxLCJlbWFpbCI6ImFsaWNlQGp1aWNlLXNoLm9wIn0sImlhdCI6MTczMDQ3NjgwMCwiZXhwIjoxNzMwNDk4NDAwfQ.signature'
      },
      auth: process.env.BRIGHT_AUTH_ID
    });
});