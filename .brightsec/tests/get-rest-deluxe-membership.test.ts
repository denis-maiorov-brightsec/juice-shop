...
test('GET /rest/deluxe-membership', { signal: AbortSignal.timeout(timeout) }, async () => {
  await runner
    ...
    .run({
      ...
      headers: {
        Authorization:
          'Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjp7ImlkIjoxLCJlbWFpbCI6ImFteUBqdWljZS1zaG9wLmNvbSIsInJvbGUiOiJjdXN0b21lciJ9LCJpYXQiOjE3MDAwMDAwMDAsImV4cCI6MTcwMDAyMTYwMH0.signature'
      },
      auth: process.env.BRIGHT_AUTH_ID
    });
});